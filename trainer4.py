...
def model_fusion(model):
    fused_model=copy.deepcopy(model)
    model.train()
    fused_model.train()
    fused_model = torch.quantization.fuse_modules(fused_model, ['conv1','bn1'], inplace=True)
    ...
    model.eval()
    fused_model.eval()
    #assert model_equivalence(model_1=model, model_2=fused_model, device=torch.device("cpu:0"), rtol=1e-03, atol=1e-06, num_tests=100, input_size=(1,3,32,32)), "Fused model is not equivalent to the original model!"
    return fused_model
